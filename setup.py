...
class Build(build):
    def run(self):
        if path.isdir("queries"):
            dest = path.join(self.build_lib, "tree_sitter_bidhuscript", "queries")
...
setup(
    ...
    package_data={
        "tree_sitter_bidhuscript": ["*.pyi", "py.typed"],
        "tree_sitter_bidhuscript.queries": ["*.scm"],
    },
    ext_package="tree_sitter_bidhuscript",
    ext_modules=[
        Extension(
            name="_binding",
            sources=[
                "bindings/python/tree_sitter_bidhuscript/binding.c",
                "src/parser.c",
            ],
            define_macros=[
